#!/usr/bin/env python3
"""
VLESS VPN Client v2.0 - Улучшенная версия
Правильная обработка ws+tls, reality, tcp серверов
"""
...
class VLESSConfig:
    ...
    def parse_vless_url(url: str) -> Optional[Dict[str, Any]]:
        """Парсит VLESS URL с правильной обработкой всех параметров"""
        try:
            ...
            # Разделяем основную часть и параметры
            if "?" not in url_part:
                return None
            
            main_part, query_string = url_part.split("?", 1)
            
            # Парсим UUID и host:port
            if "@" not in main_part:
                return None
            
            uuid, hostport = main_part.split("@", 1)
            
            # Парсим host:port (поддержка IPv6)
            ...
            # Очищаем порт от лишних символов
            port_str = port_str.split("/")[0]  # Убираем /path если есть
            try:
                port = int(port_str)
            except ValueError:
                Logger.log(f"Неверный порт в URL: {port_str}", "ERROR")
                return None
    ...
    @staticmethod
    def config_to_xray(config: Dict[str, Any], split_mode: bool = True) -> Dict[str, Any]:
        """Создаёт правильный конфиг XRay для любого типа сервера"""
        ...
        # Настраиваем streamSettings в зависимости от типа
        stream = xray_config["outbounds"][0]["streamSettings"]
        
        # TLS/Reality настройки
        if security == "tls":
            stream["tlsSettings"] = {
                "serverName": params.get("sni", config["host"]),
                "alpn": params.get("alpn", "http/1.1").split(","),
        ...
        # Добавляем правила маршрутизации для split режима
        if split_mode:
...
class XRayController:
    ...
    def start(self, server: Dict[str, Any], mode: str = "split") -> bool:
        ...
        # Генерируем конфиг
        config = VLESSConfig.config_to_xray(server, split_mode=(mode == "split"))
        config_file = XRAY_CONFIG_SPLIT if mode == "split" else XRAY_CONFIG
        
        Logger.log(f"Режим: {'РАЗДЕЛЕНИЕ (РФ напрямую)' if mode == 'split' else 'ВСЁ через VPN'}")
...
class VPNClient:
    ...
    def connect(self, mode: str = "split", auto: bool = True) -> bool:
        Logger.log("=" * 60)
        Logger.log("Запуск VPN клиента v2.0...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="VLESS VPN Client v2.0")
